...
calcSpear <- function(ecologyResults){
  sampleMetric <- lapply(split(taxaMetricValues, taxaMetricValues$SAMPLE_NUMBER), function(sample) {
    # calculate SPEAR score
    scoringTaxa <- length(sample$SPEAR_SPECIES[sample$SPEAR_SPECIES == TRUE])
    scoringTaxaLog <- log(length(sample$SPEAR_SPECIES[sample$SPEAR_SPECIES == TRUE]))
    spearRatio  <- ifelse(scoringTaxa > 0,
                            100 / scoringTaxa * scoringTaxaLog, 0)
    # regression coefficients for Toxicant Exposure calculation
    toxicantExposureCoeff <- data.frame(recoveryArea = c("absent", "presence", "unknown"),
   ...
   spearToxicantRatio <- toxicantExposureCoeff$p1[toxicantExposureCoeff$recoveryArea == recoveryArea] *
    ...
    # High:     > 44% SPEAR
    spearWaterQuality <- data.frame(class = c("Bad", "Poor", "Moderate", "Good", "High"),
                                    upperlimit = c(11, 22, 33, 44, 100),
                                    value = seq(1:5))
    # calculate SPEAR condition using psiCondition dataframe saved in package
    intervals <- data.frame(value = table(cut(spearRatio, breaks = c(0, spearWaterQuality[, "upperlimit"]),
                                              include.lowest = TRUE)))
    intervals$row <- row.names(intervals)
    # merge condition lookup table with results
    intervalCondition <- merge(spearWaterQuality, intervals, by.x = "value", by.y = "row")
    # create list of the three results: SPEAR score, SPEAR condition, SPEAR class
    spearResult <- c(spearRatio, spearToxicantRatio,
