...
SEPAresults <- SEPAresults[(!(SEPAresults$FORMATTED_ENTRY == 0 | is.na(SEPAresults$FORMATTED_ENTRY))), ]
...
riverflySum$`Caseless caddis` <- sum(SEPAresults$FORMATTED_ENTRY[SEPAresults$RIVERFLY_GROUP == "Caseless caddis"],
                                     na.rm = T)
...
riverflySum$`Blue winged olives` <- sum(
                                 SEPAresults$FORMATTED_ENTRY[SEPAresults$RIVERFLY_GROUP == "Blue winged olives"],
                                        na.rm = T)
riverflySum$`True Mayflies` <- sum(SEPAresults$FORMATTED_ENTRY[SEPAresults$RIVERFLY_GROUP == "True Mayflies"],
                                   na.rm = T)
