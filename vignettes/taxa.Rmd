...
However, there are a number of independent sources of truth for taxa concepts. These are WFD100, NBN,  ECN and  metrics (LAMM & AWIC etc) taxa lists. Furthermore, the identification keys are independently produced.

To solve this implementation dilemma, we combine all these sources of truth into a taxa table (INVERT-TAXON-DICTIONARY), and co-ordinate updates to keys, guidance, data entry, training and auditing as required. This is repeated in similar way for other lists, for example diatoms, macrophytes and marine lists. 
